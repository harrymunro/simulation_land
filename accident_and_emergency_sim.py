...
# Data to save
patient_wait_time_data = {"patient_name": [], "admittance_time":[], "wait_time": []}
accident_and_emergency_queue_data = {"queue_length": [], "simulation_time": []}

# A&E department component
# Should delete patient when they leave hospital
class AccidentEmergencyDepartment():
    ...
    def process_patient(self, patient):
        ...
        accident_and_emergency_queue_data["simulation_time"].append(self.env.now)
        ...
        patient_finish_time = self.env.now - patient_arrival_time
        ...
        patient_wait_time_data["admittance_time"].append(patient_arrival_time)
        patient_wait_time_data["wait_time"].append(patient_finish_time)
...
accident_and_emergency_department = AccidentEmergencyDepartment(env=env, capacity=1)

local_knowledge["nearest_hospital"] = accident_and_emergency_department

# Create the patients
local_population_size = 10000
...
sns.scatterplot(data=patient_wait_time_data, x="admittance_time", y="wait_time")
plt.show()

sns.scatterplot(data=accident_and_emergency_queue_data, x="simulation_time", y="queue_length")
plt.show()
